...
exp.init = function() {
    ...
    // call user-defined customization function
    this.customize();
...
exp.submit = function() {
    ...
    var flattenData = function(data) {
        ...
        var out = _.map(trials, function(t) {
            // Here the data is the general information besides the trials.
    ...
    // Organize data into a cleaner hierarchical structure
    // Separate participant-level data from trial-level data
    var participantInfo = {};
    var trialData = addEmptyColumns(exp.trial_data);
    
    // Add MTurk-specific fields if applicable
    if (config_deploy.is_MTurk) {
        var HITData = getHITData();
        participantInfo["assignment_id"] = HITData["assignmentId"];
        participantInfo["worker_id"] = HITData["workerId"];
        participantInfo["hit_id"] = HITData["hitId"];

        // creates a form with assignmentId input for the submission to MTurk
    ...
    // Extract participant-level data (demographics, post-survey, etc.)
    // These should appear once per participant, not in every trial
    var participantKeys = [
        'prolific_id', 'startDate', 'startTime', 'endTime', 'timeSpent',
        'age', 'gender', 'years_lived', 'vision', 'vision_other',
        'country', 'country_other', 'native_lang', 'native_lang_other',
        'familiar_us', 'familiar_jp', 'familiar_visual',
        'image_familiar', 'image_familiar_spec', 'post_comments'
    ];
    
    // Copy participant-level data from global_data
    for (var key in exp.global_data) {
        if (exp.global_data.hasOwnProperty(key)) {
            participantInfo[key] = exp.global_data[key];
        }
    }
    
    // Add experiment ID to participant info
    participantInfo.experiment_id = config_deploy.experimentID;
    
    // Create hierarchical structure: {participant_info: {...}, trials: [...]}
    var organizedData = {
    ...
    if (config_deploy.liveExperiment) {
        ...
        // Submit organized data (but still flatten for backward compatibility if needed)
        // For now, submit the organized structure
        submitResults(
        ...
        // For CSV export, we still need the flattened format
        // Create a temporary data structure for flattening
        var tempDataForFlatten = {
            experiment_id: participantInfo.experiment_id,
            trials: trialData
        };
        // Merge participant info for flattening (this creates the wide format for CSV)
        var dataForFlatten = _.merge({}, participantInfo, tempDataForFlatten);
        var flattenedData = flattenData(dataForFlatten);
